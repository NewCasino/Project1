...
<script language="C#" type="text/C#" runat="server">
    ...
    private List<SelectListItem> GetAuthenticGames()
    {
        var p = new Dictionary<string, object>()
        {
            { "VendorID" , new VendorID[] { VendorID.Authentic } },
        };
        int total = 0;
...
#btnRegAuthentic { float:right; }
</style>

<p>
<label class="label">Game : </label>
<%: Html.DropDownList("gameID", GetAuthenticGames(), new { @class = "ddl", @id = "ddlAuthenticGameID"}) %>
</p>
...
<%: Html.TextBox("extraParameter1", string.Empty, new { @id = "txtAuthenticTableID" })%>
</p>

<p>
<label class="label">Limit ID: </label>
<%: Html.TextBox("extraParameter2", string.Empty, new { @id = "txtAuthenticLimitID" })%>
</p>

<p style="position:absolute; bottom:0px; right:0px;width:auto;">
<button id="btnRegAuthentic">Submit</button>
</p>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#btnRegAuthentic').button().click(function (e) {
            e.preventDefault();
            $('#loading').show();
            var url = '<%= this.Url.ActionEx("RegisterTable").SafeJavascriptStringEncode() %>';
            var data = {
                gameID: $('#ddlAuthenticGameID').val(),
                category: $('#ddlLiveCasinoCategory').val(),
                extraParameter1: $('#txtAuthenticTableID').val(),
                extraParameter2: $('#txtAuthenticLimitID').val(),
