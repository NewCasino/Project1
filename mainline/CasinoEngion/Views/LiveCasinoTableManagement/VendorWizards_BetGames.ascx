...
<script language="C#" type="text/C#" runat="server">
    ...
    private List<SelectListItem> GetBetGamesGames()
    {
        var p = new Dictionary<string, object>()
        {
            { "VendorID" , new VendorID[] { VendorID.BetGames } },
        };
        int total = 0;
...
#btnRegBetGames { float:right; }
</style>

<p>
<label class="label">Game : </label>
<%: Html.DropDownList("gameID", GetBetGamesGames(), new { @class = "ddl", @id = "ddlBetGamesGameID"}) %>
</p>
...
<%: Html.TextBox("extraParameter3", string.Empty, new { @id = "txtBetGamesTableID" })%>
</p>

<p style="position:absolute; bottom:0px; right:0px; width:auto;">
<button id="btnRegBetGames">Submit</button>
</p>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#btnRegBetGames').button().click(function (e) {
            e.preventDefault();
            $('#loading').show();
            var url = '<%= this.Url.ActionEx("RegisterTable").SafeJavascriptStringEncode() %>';
            var data = {
                gameID: $('#ddlBetGamesGameID').val(),
                category: $('#ddlLiveCasinoCategory').val(),
                //extraParameter1: $('#txtBetGamesTableID').val(),
