...
<script language="C#" type="text/C#" runat="server">
    ...
    private List<SelectListItem> GetLiveGamesGames()
    {
        var p = new Dictionary<string, object>()
        {
            { "VendorID" , new VendorID[] { VendorID.LiveGames } },
        };
        int total = 0;
...
#btnRegLiveGames { float:right; }
</style>

<p>
<label class="label">Game : </label>
<%: Html.DropDownList("gameID", GetLiveGamesGames(), new { @class = "ddl", @id = "ddlLiveGamesGameID"}) %>
</p>
...
<%: Html.TextBox("extraParameter1", string.Empty, new { @id = "txtLiveGamesTableID" })%>
</p>

<p>
<label class="label">Limit ID: </label>
<%: Html.TextBox("extraParameter2", string.Empty, new { @id = "txtLiveGamesLimitID" })%>
</p>

<p style="position:absolute; bottom:0px; right:0px;width:auto;">
<button id="btnRegLiveGames">Submit</button>
</p>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#btnRegLiveGames').button().click(function (e) {
            e.preventDefault();
            $('#loading').show();
            var url = '<%= this.Url.ActionEx("RegisterTable").SafeJavascriptStringEncode() %>';
            var data = {
                gameID: $('#ddlLiveGamesGameID').val(),
                category: $('#ddlLiveCasinoCategory').val(),
                extraParameter1: $('#txtLiveGamesTableID').val(),
                extraParameter2: $('#txtLiveGamesLimitID').val(),
