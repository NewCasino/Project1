...
<script language="C#" type="text/C#" runat="server">
    ...
    private List<SelectListItem> GetLuckyStreakGames()
    {
        var p = new Dictionary<string, object>()
        {
            { "VendorID" , new VendorID[] { VendorID.LuckyStreak } },
        };
        int total = 0;
...
#btnRegLuckyStreak { float:right; }
</style>

<p>
<label class="label">Game : </label>
<%: Html.DropDownList("gameID", GetLuckyStreakGames(), new { @class = "ddl", @id = "ddlLuckyStreakGameID"}) %>
</p>
...
<%: Html.TextBox("extraParameter2", string.Empty, new { @id = "txtLuckyStreakLimitID" })%>
</p>

<p style="position:absolute; bottom:0px; right:0px;width:auto;">
<button id="btnRegLuckyStreak">Submit</button>
</p>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#btnRegLuckyStreak').button().click(function (e) {
            e.preventDefault();
            $('#loading').show();
            var url = '<%= this.Url.ActionEx("RegisterTable").SafeJavascriptStringEncode() %>';
            var data = {
                gameID: $('#ddlLuckyStreakGameID').val(),
                category: $('#ddlLiveCasinoCategory').val(),
                extraParameter1: $('#txtLuckyStreakLimitID').val(),
                extraParameter2: $('#txtLuckyStreakLimitID').val(),
