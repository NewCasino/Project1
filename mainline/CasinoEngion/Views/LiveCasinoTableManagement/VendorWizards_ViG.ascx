...
<script language="C#" type="text/C#" runat="server">
    ...
    private List<SelectListItem> GetViGGames()
    {
        var p = new Dictionary<string, object>()
        {
            { "VendorID" , new VendorID[] { VendorID.ViG } },
        };
        int total = 0;
...
#btnRegViG { float:right; }
</style>

<p>
<label class="label">Game : </label>
<%: Html.DropDownList("gameID", GetViGGames(), new { @class = "ddl", @id = "ddlViGGameID"}) %>
</p>
...
<%: Html.TextBox("extraParameter1", string.Empty, new { @id = "txtViGTableID" })%>
</p>

<p>
<label class="label">Limit ID: </label>
<%: Html.TextBox("extraParameter2", string.Empty, new { @id = "txtViGLimitID" })%>
</p>

<p style="position:absolute; bottom:0px; right:0px;width:auto;">
<button id="btnRegViG">Submit</button>
</p>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#btnRegViG').button().click(function (e) {
            e.preventDefault();
            $('#loading').show();
            var url = '<%= this.Url.ActionEx("RegisterTable").SafeJavascriptStringEncode() %>';
            var data = {
                gameID: $('#ddlViGGameID').val(),
                category: $('#ddlLiveCasinoCategory').val(),
                extraParameter1: $('#txtViGTableID').val(),
                extraParameter2: $('#txtViGLimitID').val(),
