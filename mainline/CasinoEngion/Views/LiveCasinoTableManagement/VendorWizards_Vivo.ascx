...
<script language="C#" type="text/C#" runat="server">
    ...
    private List<SelectListItem> GetVivoGames()
    {
        var p = new Dictionary<string, object>()
        {
            { "VendorID" , new VendorID[] { VendorID.Vivo } },
        };
        int total = 0;
...
<%: Html.DropDownList("gameID", GetVivoGames(), new { @class = "ddl", @id = "ddlVivoGameID"}) %>
</p>
...
<%: Html.TextBox("extraParameter1", string.Empty, new { @id = "txtVivoTableID" })%>
</p>

<p>
<label class="label">Limit ID: </label>
<%: Html.TextBox("extraParameter2", string.Empty, new { @id = "txtVivoLimitID" })%>
</p>

<p style="position:absolute; bottom:0px; right:0px;width:auto;">
<button id="btnRegVivo">Submit</button>
</p>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#btnRegVivo').button().click(function (e) {
            e.preventDefault();
            $('#loading').show();
            var url = '<%= this.Url.ActionEx("RegisterTable").SafeJavascriptStringEncode() %>';
            var data = {
                gameID: $('#ddlVivoGameID').val(),
                category: $('#ddlLiveCasinoCategory').val(),
                extraParameter1: $('#txtVivoTableID').val(),
                extraParameter2: $('#txtVivoLimitID').val(),
