...
<script language="C#" type="text/C#" runat="server">
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        url.Append(Domain.GetCfg(Eyecon.GameBaseURL));
        url.AppendFormat("?gameid={0}&brand={1}&nid={2}&sid={3}&lang={4}",
            this.Model.GameID,
            Domain.GetCfg(Eyecon.Brand),
            Domain.GetCfg(Eyecon.NID),
            Domain.GetCfg(Eyecon.SID),
            this.Language
            );
        ...
        if (!FunMode)
        {
            ...
            //uid={0}&alias={1}&gameid={2}&guid={3}&cur=GBP&mode=cash
            UserAccessor ua = UserAccessor.CreateInstance<UserAccessor>();
            string alias = ua.GetUserAlias(UserSession.UserID);
            string playerAlias = String.IsNullOrWhiteSpace(alias) ? UserSession.UserID.ToString() : alias;

            startUrl += String.Format("&uid={0}&alias={1}&guid={2}&cur={3}", UserSession.UserID, playerAlias, response.TokenKey, currency);
            mode = "cash";
        }
        else
        {
            // Currency is required to launch game in fun mode
            startUrl += "&cur=EUR";
        }
        startUrl += "&mode=" + mode;
...
<body>
    ...
    <%=InjectScriptCode(Eyecon.CELaunchInjectScriptUrl) %>
