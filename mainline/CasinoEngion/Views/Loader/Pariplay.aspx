...
<script language="C#" type="text/C#" runat="server">
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        url.Append(Domain.GetCfg(Pariplay.GameBaseURL));
        url.AppendFormat("?gameCode={0}&language={1}&operatorId={2}&freePlay={3}&mobile={4}&clientId={5}&mode={6}",
            this.Model.GameID,
            this.Language,
            Domain.GetCfg(Pariplay.PartnerID),
            FunMode ? "true" : "false",
            mobileDevice ? "true" : "false",
            Domain.GetCfg(Pariplay.ClientID),
            Domain.GetCfg(Pariplay.Mode)
            );
        ...
        {
            if (!string.IsNullOrWhiteSpace(Domain.DomainDefaultCurrencyCode))
            {
                startUrl += String.Format("&currencyCode={0}", Domain.DomainDefaultCurrencyCode);
            }
            else
            {
                startUrl += "&currencyCode=EUR";
            }
        }
...
<body>
    ...
    <%=InjectScriptCode(Pariplay.CELaunchInjectScriptUrl) %>
