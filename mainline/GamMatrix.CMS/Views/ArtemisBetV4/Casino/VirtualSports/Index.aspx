...
<script type="text/C#" runat="server">
    private string GetLoggedInHtml() {
        if (!Profile.IsAuthenticated)
            return string.Empty;
        return this.GetMetadata("/Casino/Hall/_Index_aspx.LoggedInHtml").HtmlEncodeSpecialCharactors();
    }
...
<div class="Breadcrumbs" role="navigation">
    <ul class="BreadMenu Container" role="menu">
        ...
        <li class="BreadItem <%=isFinal()%>" role="menuitem" itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="itemscope">
            <a class="BreadLink url" href="<%= this.GetMetadata("/Metadata/Breadcrumbs/Home/CasinoVirtualSports/.Url") %>" itemprop="url" title="<%= this.GetMetadata("/Metadata/Breadcrumbs/Home/CasinoVirtualSports/.Title") %>">
                <span itemprop="title"><%= this.GetMetadata("/Metadata/Breadcrumbs/Home/CasinoVirtualSports/.Name") %></span>
            </a>
        </li>
        <%=GetCategory()%>
    </ul>
...
<div class="Framework" id="framework">
    ...
    <div class="Zone Container Intro AllSlidersContainer">
        <div class="sliderContainer">
            <% Html.RenderPartial("/Components/Slider", this.ViewData.Merge(new { @SliderPath = "/Metadata/Sliders/VirtualSports" })); %> 
        ...
        <div class="SliderButtons">    
            <%=this.GetMetadata("/Casino/Hall/_Index_aspx.Banner_Controller")%>
        </div>
    </div>
    ...
    <main class="MainContent">
        ...
        <% Html.RenderPartial("/Casino/Hall/GameNavWidget/Main", this.ViewData.Merge( new {@InitialSliderCategoryCount = "3", @isGlobalbetGames=true } )); %>
...
<ui:MinifiedJavascriptControl runat="server">
    <script type="text/javascript">
        ...
        function receiveMessage(event) {
            var bodyHeight = event.data.bodyHeight;
            if (bodyHeight != null) {
                $('.GameLoaderIframe').height(bodyHeight);
            }
    ...
    //        var activeCat = $('li.ActiveCat').find('.CatText').html();
        ...
        $(function () {
            ...
            $('.InlineGameBox').insertAfter('.AllGames .GamesContainer');

            $('.GFInline .GFText').click();
    
            $(document).on('INLINE_GAME_OPENED', function (evt, data) {
                $('.AllGames .GamesContainer').hide();
                window.scrollTo(0, 0);
            });
            $(document).bind('INLINE_GAME_CLOSED', function () {
                $('body').removeClass('InlineGamePage');
                $('.AllGames .GamesContainer').show();
            });
    ...
    //    $(".PopupsContainer").remove();
        ...
        $(document).on('GAME_MULTI_POPUP_INITIALIZED', function (event, gameMOWidget) {
            ...
            if ($("#txtQuickDepositAmount").length == 1 && $("li.CBQuickDeposit .customCover").length == 0) {
                $("<div class='customCover'><span class='icon'>+</span><span class='label'><%=this.GetMetadata("/Casino/Hall/_Index_aspx.QuicklyDeposit").SafeJavascriptStringEncode() %></span></div>").appendTo("li.CBQuickDeposit").click(function () {
