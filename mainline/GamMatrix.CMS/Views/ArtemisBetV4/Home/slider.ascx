...
<script type="text/C#" runat="server">
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        this.SliderPaths = Metadata.GetChildrenPaths((this.ViewData["SliderPath"] as string))
                .Where( p => !IsExcludedByCountry(p)).ToArray();
        base.OnInit(e);
    }
...
<script type="text/javascript">
    $(function () {
        ...
        var timerHandler = function () {
            if (_isMouseEnter || $("#show-slider:visible").length > 0) {
                _timer = setTimeout(timerHandler, 1000);
                return;
            } 
            _timer = null;
        ...
        $('a.Badge', $slider).click(function (e) {
            ...
            if( $('a.ActiveBadge', $slider).length < 1 ){
                $('a.Badge', $slider).eq(0).addClass('ActiveBadge');
                //return;
            }else{                
                currentSliderData = $('a.ActiveBadge', $slider).data("slide");
            }
            //console.log(currentSliderData);
            if(currentSliderData >= $('a.Badge', $slider).length){
                currentSliderData = 0;
            }
            var currentSlider = $('a.Badge', $slider).eq(currentSliderData);
            $('a.ActiveBadge', $slider).removeClass('ActiveBadge');
            currentSlider.addClass('ActiveBadge');
            var marginLeft = (currentSlider.data('slide') - 1) * $slider.width();
