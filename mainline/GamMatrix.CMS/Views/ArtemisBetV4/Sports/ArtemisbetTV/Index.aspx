    ...
    <div class="Breadcrumbs" role="navigation">
        <ul class="BreadMenu Container" role="menu">
            ...
            <li class="BreadItem BreadCurrent" role="menuitem" itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="itemscope">
                <a class="BreadLink url" href="<%= this.GetMetadata("/Metadata/Breadcrumbs/Home/ArtemisbetTV/.Url") %>" itemprop="url" title="<%= this.GetMetadata("/Metadata/Breadcrumbs/Home/ArtemisbetTV/.Title") %>">
                    <span itemprop="title"><%= this.GetMetadata("/Metadata/Breadcrumbs/Home/ArtemisbetTV/.Name") %></span>
                </a>
            </li>
        </ul>
    </div>

    <% Html.RenderPartial( "../Iframe", this.ViewData.Merge( new { ConfigrationItem = "OddsMatrix_ArtemisbetTVPage", PercentageWidth = "100"})); %>

    <ui:MinifiedJavascriptControl runat="server" AppendToPageEnd="true">
        <script type="text/javascript">
            document.domain = $("body").data("cookiedomain");
            $(function() {
                $('body').addClass('ArtemisbetTVPage');
            });
        </script>
