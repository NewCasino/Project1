    ...
    <div class="Breadcrumbs" role="navigation">
        <ul class="BreadMenu Container" role="menu">
            ...
            <li class="BreadItem BreadCurrent" role="menuitem" itemtype="http://data-vocabulary.org/Breadcrumb" itemscope="itemscope">
                <a class="BreadLink url" href="<%= this.GetMetadata("/Metadata/Breadcrumbs/Home/VirtualSports/.Url") %>" itemprop="url" title="<%= this.GetMetadata("/Metadata/Breadcrumbs/Home/VirtualSports/.Title") %>">
                    <span itemprop="title"><%= this.GetMetadata("/Metadata/Breadcrumbs/Home/VirtualSports/.Name") %></span>
                </a>
            </li>
        </ul>
    </div>

    <% Html.RenderPartial( "../Iframe", this.ViewData.Merge( new { ConfigrationItem = "OddsMatrix_VirtualPage", PercentageWidth = "100"})); %>

    <ui:MinifiedJavascriptControl runat="server" AppendToPageEnd="true">
        <script type="text/javascript">
            document.domain = $("body").data("cookiedomain");
            $(function() {
                $('body').addClass('VirtualSportsPage');
            });
        </script>
