...
scriptEmail.Visible = Model.InputSettings.IsEmailVisible;

fldDOB.Visible = Model.InputSettings.IsBirthDateVisible;
scriptDOB.Visible = Model.InputSettings.IsBirthDateVisible;

fldPersonalID.Visible = true;
scriptPersonalID.Visible = true;

        if (!string.IsNullOrWhiteSpace(Model.InputSettings.PersonalID))
        {
            fldPersonalID.Visible = false;
            scriptPersonalID.Visible = false;
        }
        if (!string.IsNullOrEmpty(Request.Form["email"]))
        {
            user_email.Value = Request.Form["email"].ToString();
        }
base.OnPreRender(e);
}
...
<input type="hidden" runat="server" id="user_email" value="" class="user_email" />
<ul class="FormList">
<%------------------------------------------
Title
-------------------------------------------%>
...
<ui:MinifiedJavascriptControl runat="server" ID="scriptEmail" AppendToPageEnd="true" Enabled="true">
<script type="text/javascript">
$(function () {
$("#registerEmail").val($(".user_email").val());
});
</script>
</ui:MinifiedJavascriptControl>
<%------------------------------------------
...
$('#registerBirth').val(year + '-' + month + '-' + day);
return true;
}
</script>
</ui:MinifiedJavascriptControl>
...
<%------------------------------------------
        Personal ID
        -------------------------------------------%>
<li class="FormItem" id="fldPersonalID" runat="server">
<label class="FormLabel" for="personalID"><%= this.GetMetadata(".PersonalID_Label").SafeHtmlEncode()%></label>
        <%: Html.TextBox("personalID", Model.InputSettings.PersonalID, new Dictionary<string, object>()  
        { 
            { "class", "FormInput" },
...
function validatePersonalID() {
    ...
    if(!ret.test(value)){
        $("#registerPersonalID").parent().removeClass("OK").addClass("Error");
        return '<%= this.GetMetadata(".PersonalID_Illegal").SafeJavascriptStringEncode() %>';
    }
    $("#registerPersonalID").parent().removeClass("Error").addClass("OK");
    return true;
}
...
<li class="FormItem fldFavoriteTeam" id="fldFavoriteTeam" runat="server">
<label class="FormLabel" for="txtFavoriteTeam"><%= this.GetMetadata(".FavoriteTeam_Label").SafeHtmlEncode()%></label>

<%: Html.DropDownList("favoriteTeam", GetFavoriteTeamList(), new Dictionary<string, object>()
        { 
            { "class", "FormInput" },
            { "id", "txtFavoriteTeam" },
            { "required", "required" },
