        ...
        foreach (KeyValuePair<string, List<LiveCasinoTable>> category in categories)
        {
            ...
            foreach (LiveCasinoTable table in category.Value)
            {
                ...
                json.AppendFormat(CultureInfo.InvariantCulture
                    , "{{\"ID\":{0},\"P\":{1},\"V\":\"{2}\",\"G\":\"{3}\",\"I\":\"{4}\",\"F\":{5},\"R\":{6},\"S\":\"{7}\",\"N\":{8},\"T\":{9},\"H\":{10},\"O\":{11},\"C\":\"{12}\",\"Opened\":{13},\"Limit\":\"{14}\",\"OpeningHours\":\"{15}\",\"SeatsMax\":{16},\"SeatsTaken\":{17},\"SeatsAvailable\":{18},\"SeatsUnlimited\":{19}}},"
                    , table.ID
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        if (isLoggedIn && gameIdToOpen != null){
            $.cookie('loginDialogGameId', null);
            //debugger;
            $('.TablesList [data-tableid="' + gameIdToOpen + '"].GLItem a.GameThumb').click();
            $('#livecasino-game-popup a.PlayNowButton').click();
