...
<script language="C#" type="text/C#" runat="server">
    ...
    { 
        get 
        {
            bool isVisible = GamMatrixClient.GetGamingVendors()
                .Exists(v => v.VendorID == GamMatrixAPI.VendorID.BingoNetwork ||
                    v.VendorID == GamMatrixAPI.VendorID.EverleafNetwork);

            if (Settings.Registration.UsenameAsAlias)
                isVisible = false;
            if (!Settings.Registration.IsAliasVisible)
                isVisible = false;
            if (this.Model != null && !string.IsNullOrWhiteSpace(this.Model.Alias))
                isVisible = false;
            return isVisible; 
        } 
    }
    ...
    private bool IsCurrencyVisible { get { return this.Model == null; } }
...
<script type="text/javascript">
    $(function () {
        ...
        if ($('#ddlCurrency > option').length <= 1)
            $('#fldCurrency').hide();
    });
</script>
</ui:MinifiedJavascriptControl>
