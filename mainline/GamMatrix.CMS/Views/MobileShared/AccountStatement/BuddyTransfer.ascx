...
<script runat="server">
	public string GetDescription(TransInfoRec trans, string format)
	{
		...
		<table class="DetailTable Cols-4">
			...
			<tr>
				<td class="col-account">
					<span class="TransDebit"><%= GetDebitAccountName(trans, this.GetMetadata(".FromAccount")).SafeHtmlEncode() %></span>
					<span class="TransCredit"><%= GetCreditAccountName(trans, this.GetMetadata(".ToAccount")).SafeHtmlEncode()%></span>
				</td>
				<td>
					<span class="TransDebit">-<%= GetDebitAmount(trans) %></span>
					<span class="TransCredit"><%= GetCreditAmount(trans) %></span>
				</td>
				<td><%= GetDescription(trans, this.GetMetadata(trans.UserID == Profile.UserID ? ".TransferToPlayer" : ".TransferFromPlayer")).SafeHtmlEncode()%></td>
