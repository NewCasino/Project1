    ...
    <Tabs>
        ...
        <ui:Panel runat="server" ID="tabFindFriend" Caption="<%$ Metadata:value(.Tab_FindFriend) %>"  Selected="true">
            <form id="formFindFriend" onsubmit="return false" 
                ...
                <center>
                    <button type="submit" class="Button RegLink DepLink NextStepLink" id="btnFindFriend">
                        <span class="ButtonText"><%= this.GetMetadata(".Button_Continue").SafeHtmlEncode()%></span>
                    </button>  
                </center>
            </form>
        ...
        <form id="formChooseFriend" onsubmit="return false" method="post" action="<%= this.Url.RouteUrl("BuddyTransfer", new { @action = "ValidateFriend" }).SafeHtmlEncode() %>" enctype="application/x-www-form-urlencoded">
            ...
            <ui:InputField ID="fldFriend" runat="server" ShowDefaultIndicator="true" BalloonArrowDirection="Left">
                ...
                <ul id="paycards-selector" class="FormList">
                    <% foreach (var entry in this.Friends)
                       {
                           %>
                    <li  class="FormItem">
                        <input type="radio" name="friendUserID" value="<%= entry.Key %>" id="friend_<%= entry.Key %>" class="FormRadio" />
            ...
            <center>
                <button type="submit" class="Button RegLink DepLink NextStepLink" id="btnTransferToExistingFriend">
                        <span class="ButtonText"><%= this.GetMetadata(".Button_Continue").SafeHtmlEncode()%></span>
                </button> 
            </center>
...
<script language="javascript" type="text/javascript">
    $(function(){
        $("#tabbedFriends .tabbody").hide();
        $("#tabbedFriends .tabbody").eq(0).show();
    });
    function __initializeTabbedContent( obj ){
        $(".tab a" , obj).click(function(){
            $(".tabbody" , obj).hide();
            $($(this).attr("href") , obj).show();
            $(".tab" , obj).removeClass("selected");
