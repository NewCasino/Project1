...
<script type="text/C#" runat="server">
    ...
    private string GetFormProfix()
    {
        if (Settings.Limitation.Deposit_MultipleSet_Enabled)
        {
            return ViewData["Period"].ToString();
        }
        else
            return string.Empty;
        //if (this.Model != null)
        //    return this.Model.Period.ToString();
        //if (ViewData["Period"] != null)
        //    return ViewData["Period"] as string;
        //return string.Empty;
    }
...
<form id="depositLimit<%=GetFormProfix() %>" class="FormList DepositLimitForm" action="<%= Url.RouteUrl("DepositLimit", new { @action = "Apply", @depositLimitPeriod = SpecifiedPeriod }, Request.IsHttps() ? "https" : "http") %>" method="post">
    ...
    <fieldset>
        ...
        <ul class="FormList DepositLimitList">
             <li class="FormItem LimitCurrencyItem">
                ...
                { 
                { "class", "FormInput" },
                { "id", "limitCurrency"+GetFormProfix() },
                { "required", "required" },
            ...
            <li class="FormItem LimitAmountItem">
                ...
                { 
                { "class", "FormInput" },
                { "id", "limitAmount"+GetFormProfix() },
                { "required", "required" },
                { "maxlength", "30" },
                { "placeholder", this.GetMetadata(".Amount_Choose") },
                { "data-validator", ClientValidators.Create().Required(this.GetMetadata(".Amount_Empty")).Custom("InputView"+GetFormProfix()+".validateAmount") }
                }) %>
                <span class="FormStatus">Status</span>
                <span class="FormHelp"></span>
            </li>
            ...
            <li class="FormItem PeriodItem">
                ...
                <ul class="FormList" id="limitPeriod">
                    <%if (Settings.Limitation.Deposit_MultipleSet_Enabled)
                        { %>
                   <% if (SpecifiedPeriod != RgDepositLimitPeriod.None && SpecifiedPeriod == RgDepositLimitPeriod.Daily)
                        { %>
                    <li class="FormItem">
                    ...
                    <% if (SpecifiedPeriod != RgDepositLimitPeriod.None && SpecifiedPeriod == RgDepositLimitPeriod.Weekly)
                        { %>
                    <li class="FormItem">
                    ...
                    <% if (SpecifiedPeriod != RgDepositLimitPeriod.None && SpecifiedPeriod == RgDepositLimitPeriod.Monthly)
                        { %>
                    <li class="FormItem">
...
<script type="text/javascript">
    function InputView<%=GetFormProfix() %>() {
        new CMS.views.AmountInput('#limitAmount<%=GetFormProfix() %>');

        $('#limitRemove<%=GetFormProfix() %>').click(function () {
            $('#depositLimit<%=GetFormProfix() %>').attr('action', '<%= this.Url.RouteUrl( "DepositLimit", new { @action="Remove", @depositLimitPeriod = SpecifiedPeriod } ).SafeJavascriptStringEncode() %>');
        });

        new CMS.mobile360.views.Form("#depositLimit<%=GetFormProfix() %>");
    }

    InputView<%=GetFormProfix() %>.validateAmount = function () {
