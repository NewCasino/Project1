...
<script runat="server">
    ...
    private string CssClass
    {
        get
        {
            if (ViewData["CssClass"] == null)
                return string.Empty;
            return ViewData["CssClass"] as string;
        }
    }
    ...
    private string GameID
    {
        get
        {
            if (ViewData["GameID"] == null)
                return string.Empty;
            return ViewData["GameID"] as string;
        }
    }
...
<div class="GameRecommendedGamesWrap <%=InternalClass %> <%=CssClass %>">
 <script type="text/javascript">
     function getGameRecommendedGames(callBack) {
         $.getJSON("/Casino/Hall/GetGameRecommendedGamesJson?ids=<%=GameID%>", function (json) {
     ...
     function getGameRecommendedWidgetSize(containerId) {
         return {
             max: {
                 useSize: true,
                 w: 200,
    ...
    <%if (!IsHorizontal)
            { %>
    <% Html.RenderPartial("/Components/GamesVerticalSlider", this.ViewData.Merge(new { @ContainerClass = "GameRecommendedGames", @Title = GetTitle(),@ComputeSizeFun ="getGameRecommendedWidgetSize",  @GetGamesDataFun = "getGameRecommendedGames",@OpenGameDirectly = true })); %>
    <style type="text/css">
        <%=this.GetMetadata(".CSS_Vertical") %>
    </style>
