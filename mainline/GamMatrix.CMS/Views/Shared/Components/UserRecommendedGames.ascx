...
<div class="UserRecommendedGamesWrap <%=InternalClass %> <%=CssClass %>">
 <script type="text/javascript">
     function getUserRecommendedGames(callBack) {
         $.getJSON("/Casino/Hall/GetUserRecommendedGamesJson", function (json) {
    ...
    <%if (IsHorizontal)
            { %>
    <% Html.RenderPartial("/Components/GamesSlider", this.ViewData.Merge(new { @ContainerClass = "UserRecommendedGames", @Title = GetTitle(), @GetGamesDataFun = "getUserRecommendedGames",@OpenGameDirectly = true })); %>
    <style type="text/css">
        <%=this.GetMetadata(".CSS_Horizontal") %>
    </style>
