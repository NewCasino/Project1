...
<ui:TabbedContent ID="tabbedPayCards" runat="server">
    <tabs>
        ...
        <ui:Panel runat="server" ID="tabRecentCards"  IsHtmlCaption="true" Selected="true">
            <form id="formAPXPayCard" method="post" action="<%= this.Url.RouteUrl("Deposit", new { @action = "RegisterPayCard", @vendorID=this.Model.VendorID }).SafeHtmlEncode() %>" enctype="application/x-www-form-urlencoded">
                <%: Html.Hidden("bankName", this.Model.SubCode) %>
                <%: Html.Hidden("apxPayCardID", ((ExistingPayCard == null) ? string.Empty : ExistingPayCard.ID.ToString(CultureInfo.InvariantCulture))) %>

                <center>
                    <%: Html.Button(this.GetMetadata(".Button_Continue"), new { @id="btnDepositWithAPXPayCard", @class="ContinueButton button" })%>
                </center>
            </form>
...
<script language="javascript" type="text/javascript">
    ...
    $(document).ready(function () {
        $('#formAPXPayCard').initializeForm();

        $('#btnDepositWithAPXPayCard').click(function (e) {
            e.preventDefault();
            if (!isDepositInputFormValid() || !$('#formAPXPayCard').valid())
                return false;

            $(this).toggleLoadingSpin(true);

            var payCardID = $('input[name="apxPayCardID"]').val();
            if (payCardID.length > 0) {
                // <%-- post the prepare form --%>   
                tryToSubmitDepositInputForm(payCardID, function () {
                    $('#btnDepositWithAPXPayCard').toggleLoadingSpin(false);
                });
                return;
            }
            ...
            var options = {
                ...
                success: function (json) {
                    ...
                    $('input[name="apxPayCardID"]').val(json.payCardID);
            ...
            $('#formAPXPayCard').ajaxForm(options);
            $('#formAPXPayCard').submit();
        });
