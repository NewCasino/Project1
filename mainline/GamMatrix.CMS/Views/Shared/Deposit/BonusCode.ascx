...
<script type="text/javascript">
    $(function(){
        <%--var gamingAccountArray = self.tableData["table_gamming_account"];
        var venderLis = $(".BonusCode_Vendor li");
        var showCount = 0;
        var showList = [];
        $.each(venderLis,function(){
            var $li = $(this);
            var venderId = $("input",$li).val();
            var isShow = false;
            for (var k in gamingAccountArray) {
                if(venderId == gamingAccountArray[k].VendorID){
                    isShow = true;
                    showCount++;
                    break;
                }
            ...
            if (venderId == '<%= VendorID.BetConstruct.ToString() %>')
                isShow = true;
            <% } %>
            $("input",$li).removeAttr('checked');
            if(!isShow){
                $li.hide();
            }else{
                showList.push($li);
        ...
        if(showCount == 1 ){
            $("input",showList[0]).attr("checked","checked");
        }--%>
        var radioList = $(".BonusCode_Vendor input[type='radio']");
...
<div class="BonusContainer" id="<%= this.ID %>" style="display:none">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        <% if (Settings.IsOMSeamlessWalletEnabled || Settings.IsBetConstructWalletEnabled) { %>
        $('input[name="bonusCodeVendor"]', $c).click(function (e) {
