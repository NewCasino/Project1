...
<% if( Profile.IsAuthenticated )
   { %>
<style type="text/css">
<%=this.GetMetadata(".Registered_Css") %>
</style>
...
<% Html.RenderPartial("/Deposit/UKTermsConditions", this.ViewData); %>
...
//<![CDATA[
    $(document).ready(function () {
        ...
        <% if( Profile.IsAuthenticated )
           { %>
                var url = '<%= this.Url.RouteUrl( "Deposit", new { @action="GetUserAllPayCards" }).SafeJavascriptStringEncode() %>';
