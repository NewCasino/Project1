...
<script language="C#" runat="server" type="text/C#">
    ...
    private SelectList GetContractValidity()
    {
        ...
        if (this.Model != null) 
        {
            GamMatrixAPI.GetUserLicenseLTContractValidityRequest request = GmCore.GamMatrixClient.GetUserLicenseLTContractValidityRequest(this.Model.ID);
            if (request != null && request.LastLicense != null)
            {
                selectValue = ((int)request.LastLicense.ContractValidityPeriod).ToString();
            }
        }
        return new SelectList(list, "Key", "Value", selectValue);
    }
            ...
            if(isDKPersonal){
                ...
                $(document).bind('COUNTRY_SELECTION_CHANGED', function (e, data) {
                    ...
                    if( rule != null ) {
                        ...
                        if(rule.PersonalIDDisplayName!="")
                            $('#fldPersonalID .inputfield_Label').text(rule.PersonalIDDisplayName);
            
                        $('#fldPersonalID').show();
                    }
                    else{
                        __isPersonalIdMandatory = false;
                        __personalIdValidationRegularExpression = null;
                        $('#txtPersonalID').removeAttr('maxlength');
                        $('#fldPersonalID').hide();
                    }
                    if(!data.IsAutomatic)
...
<%------------------------------------------
    contract validity
 -------------------------------------------%>
<% if (Settings.EnableContract)
    { %>
 <ui:InputField ID="fldContractValidity" runat="server" ShowDefaultIndicator="true" BalloonArrowDirection="Left">
    <LabelPart><%= this.GetMetadata(".ContractValidity_Label").SafeHtmlEncode()%></LabelPart>
    <ControlPart>
        <%: Html.DropDownList("contractValidity", GetContractValidity(), new 
        {
            @id = "ddlContractValidity",
            @validator = ClientValidators.Create().Required(this.GetMetadata(".ContractValidity_Empty"))
        })%>
    </ControlPart>
</ui:InputField>
<% } %>