...
<ui:Panel runat="server" ID="pnCoolOff">
<div class="presentation">
<%= this.GetMetadata( this.CoolOffEnabled ? ".Presentation" : ".Presentation_NoOption").HtmlEncodeSpecialCharactors() %>
</div>
<%
   using (Html.BeginRouteForm("SelfExclusion", new { @action = "ApplyCoolOff" }, FormMethod.Post, new { @id = "formCoolOff" }))
   {  %>

<ui:InputField ID="fldCoolOffReason" runat="server" ShowDefaultIndicator="true" BalloonArrowDirection="Left">
	<LabelPart><%= this.GetMetadata(".CoolOffReason_Label").SafeHtmlEncode() %></LabelPart>
	<ControlPart>
...
<ui:InputField ID="fldUnsatisfiedReason" runat="server" ShowDefaultIndicator="true" BalloonArrowDirection="Left">
	<LabelPart><%= this.GetMetadata(".UnsatisfiedReason_Label").SafeHtmlEncode() %></LabelPart>
	<ControlPart>
...
<ui:InputField ID="fldCoolOffDescription" runat="server" ShowDefaultIndicator="true" BalloonArrowDirection="Left">
	<LabelPart><%= this.GetMetadata(".CoolOffReasonDescription_Label").SafeHtmlEncode() %></LabelPart>
	<ControlPart>
...
<%     
    string periodName;
    string title;
    foreach (SelfExclusionPeriod selfExclusionPeriod in AvailableCoolOffPeriods)
    {
        ...
        <tr class="<%=periodName %>">
            <td>
                <input type="radio" name="coolOffPeriod" value="<%=periodName %>" id="option<%=periodName %>" />
            </td>
            <td>
                ...
                <ui:InputField ID="fldCoolOffUntilSelectedDate" runat="server" ShowDefaultIndicator="true" BalloonArrowDirection="Left" >
                    <LabelPart Visible="false"></LabelPart>
	                <ControlPart>
                ...
                <script type="text/ecmascript">
                    ...
                    $(function () {
                        ...
                        function showCoolOffUntilSelectedDate() {
                            if ($('#option<%=periodName %>').prop('checked')) {
                                $('#fldCoolOffUntilSelectedDate').show();
                            }
                            else {
                                $('#fldCoolOffUntilSelectedDate').hide();
                            }
                        }
                ...
                <%--<input type="text" value="" validator="{date:true,messages:{date:&quot;<%=this.GetMetadata(".Date_Invalid").SafeJavascriptStringEncode() %>&quot;}}" name="selectedDate" id="coolOffUntilSelectedDate" class="textbox" autocomplete="off" readonly="readonly">                --%>
                <% } %>
            </td>
        </tr>
        <tr>
...
<br />
<center>
    <%: Html.Button(this.GetMetadata(".Button_Submit"), new { @id = "btnApplyCoolOff" })%>
</center>
    <br />
...
<script language="javascript" type="text/javascript">
    $(function () {
        ...
        $('#btnApplyCoolOff').click(function (e) {
            ...
            if (!window.confirm(initCoolOffWarningMSG())) return;
            ...
            var options = {
                ...
                success: function (html) {
                    $('#btnApplyCoolOff').toggleLoadingSpin(false);
