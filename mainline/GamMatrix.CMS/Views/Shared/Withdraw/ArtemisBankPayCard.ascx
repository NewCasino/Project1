...
<ui:TabbedContent ID="tabbedPayCards" runat="server">
    <Tabs>
        ...
        action="<%= this.Url.RouteUrl("Withdraw", new { @action = "RegisterArtemisBankPayCard" }).SafeHtmlEncode() %>" enctype="application/x-www-form-urlencoded">
...
<script type="text/javascript">
    ...
    function __loadRecentPayCards(payCardID) {
        ...
        var url = '<%= this.Url.RouteUrl( "Withdraw", new { @action="GetArtemisBankPayCards" }).SafeJavascriptStringEncode() %>';
    ...
    $(function () {
        ...
        __populatePayCards( <% Html.RenderAction("GetArtemisBankPayCards");  %> );
