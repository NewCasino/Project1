...
<ui:TabbedContent ID="tabbedPayCards" runat="server">
    ...
    <ui:Panel runat="server" ID="tabRecentCards" Caption="<%$ Metadata:value(.Tab_ExistingPayCards) %>">
        ...
        <form id="formMoneybookersPayCard" onsubmit="return false">
            ...
            <center>
                ...
                <%: Html.Button(this.GetMetadata(".Button_Continue"), new { @id="btnWithdrawWithMoneybookersPayCard" })%>
            </center>
        </form>
    ...
    <ui:Panel runat="server" ID="tabRegister" Caption="<%$ Metadata:value(.Tab_RegisterPayCard) %>">
        <form id="formRegisterMoneybookersPayCard" method="post" action="<%= this.Url.RouteUrl("Deposit", new { @action = "RegisterPayCard", @vendorID=this.Model.VendorID }).SafeHtmlEncode() %>" enctype="application/x-www-form-urlencoded">
            <%---------------------------------------------------------------
                    Register a card
            ----------------------------------------------------------------%>
            <ui:InputField ID="fldRegisterPayCard" runat="server" ShowDefaultIndicator="true" BalloonArrowDirection="Left">
	            <LabelPart><%= this.GetMetadata(".MoneybookersEmail").SafeHtmlEncode()%></LabelPart>
	            <ControlPart>
                    <%: Html.TextBox("identityNumber", "", new 
                        { 
                            @maxlength = 50,
                            @dir = "ltr",
                            @validator = ClientValidators.Create().Required(this.GetMetadata(".MoneybookersEmail_Empty"))
...
$(document).ready(function () {
    ...
    $('#formRegisterMoneybookersPayCard').initializeForm();

    __populatePayCards( <% Html.RenderAction("GetPayCards", new { vendorID = this.Model.VendorID });  %> );

    $('#btnWithdrawWithMoneybookersPayCard').click(function (e) {
        e.preventDefault();

        if (!isWithdrawInputFormValid() )
            return;

        $('#btnWithdrawWithMoneybookersPayCard').toggleLoadingSpin(true);
        tryToSubmitWithdrawInputForm($('#fldExistingPayCard input[name="existingPayCardID"]').val()
        , function () { $('#btnWithdrawWithMoneybookersPayCard').toggleLoadingSpin(false); });
    });
    ...
    $('#btnRegisterPayCard').click(function (e) {
        ...
        if (!isWithdrawInputFormValid() || !$('#formRegisterMoneybookersPayCard').valid())
            return;
        ...
        $('#formRegisterMoneybookersPayCard').ajaxForm(options);
        $('#formRegisterMoneybookersPayCard').submit();
    });
});
