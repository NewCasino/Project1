...
<ui:TabbedContent ID="tabbedPayCards" runat="server">
    <Tabs>
        ...
        <ui:Panel runat="server" ID="tabRecentCards" Caption="<%$ Metadata:value(.Tab_ExistingPayCards) %>">
            <form id="formUiPasPayCard" onsubmit="return false">
                ...
                <center>
                    ...
                    <%: Html.Button(this.GetMetadata(".Button_Continue"), new { @id="btnWithdrawWithUiPasPayCard" })%>
                </center>
            </form>
        </ui:Panel>
        ...
        <ui:Panel runat="server" ID="tabRegister" Caption="<%$ Metadata:value(.Tab_RegisterPayCard) %>">
            <form id="formRegisterUiPasPayCard" method="post" action="<%= this.Url.RouteUrl("Deposit", new { @action = "RegisterPayCard", @vendorID=this.Model.VendorID }).SafeHtmlEncode() %>" enctype="application/x-www-form-urlencoded">
            <%---------------------------------------------------------------
                    Register a card
            ----------------------------------------------------------------%>
...
<script language="javascript" type="text/javascript">
    $(document).ready(function () {
        ...
        $('#formRegisterUiPasPayCard').initializeForm();

        __populatePayCards( <% Html.RenderAction("GetPayCards", new { vendorID = this.Model.VendorID });  %>);

        $('#btnWithdrawWithUiPasPayCard').click(function (e) {
            e.preventDefault();

            if (!isWithdrawInputFormValid() || !$('#formUiPasPayCard').valid())
                return;

            $('#btnWithdrawWithUiPasPayCard').toggleLoadingSpin(true);
            tryToSubmitWithdrawInputForm($('#fldExistingPayCard input[name="existingPayCardID"]').val()
            , function () { $('#btnWithdrawWithUiPasPayCard').toggleLoadingSpin(false); });
        });
        ...
        $('#btnRegisterPayCard').click(function (e) {
            ...
            if (!isWithdrawInputFormValid() || !$('#formRegisterUiPasPayCard').valid())
                return;
            ...
            $('#formRegisterUiPasPayCard').ajaxForm(options);
            $('#formRegisterUiPasPayCard').submit();
        });
    });
