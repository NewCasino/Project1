...
<div class="ui-widget" id="vendor-operation-netent">
	<div style="margin-top: 20px; padding: 0 .7em;" class="ui-state-highlight ui-corner-all"> 
		...
		Click <strong><a id="lnkSyncNetEntGames" href="#">here</a></strong> to synchronize the lastest game list from NetEnt Web Service.
	</div>
</div>

<script language="javascript" type="text/javascript">
    $('#lnkSyncNetEntGames').click(function (e) {
        e.preventDefault();
        if (self.startLoad) self.startLoad();

        var url = '<%= this.Url.RouteUrl("CasinoMgt", new { @action = "SyncNetEntGames", @distinctName = this.Model.DistinctName.DefaultEncrypt() }).SafeJavascriptStringEncode() %>';
...
<% }
   else if( Profile.AsCustomProfile().IsInRole("CMS System Admin") )
   { %>
