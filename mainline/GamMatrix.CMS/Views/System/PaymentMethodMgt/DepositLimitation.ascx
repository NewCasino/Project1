...
<%
   ...
   }, FormMethod.Post, new { @id = "formSaveDepositLimitation" }))
   { %>

<ui:Fieldset runat="server" Legend="Deposit Limitation">
        ...
        foreach( var item in list)
        {
            ...
            %>
                ...
                <tr>
            	    ...
            	    <td><%: Html.TextBox(string.Format("depositLimitMin_{0}", item.ISO4217_Alpha)
                     ...
                     <td>
                     <%: Html.TextBox( string.Format("depositLimitMax_{0}", item.ISO4217_Alpha)
...
<%: Html.Button("Save", new { @id = "btnSaveDepositLimitation", @type = "submit" })%>
</div>

<% } %>

<script type="text/javascript">
    $('#formSaveDepositLimitation').initializeForm();

    $('#btnSaveDepositLimitation').click(function (e) {
        e.preventDefault();
        var options = {
            type: 'POST',
            dataType: 'json',
        ...
        $('#formSaveDepositLimitation').ajaxForm(options);
        $('#formSaveDepositLimitation').submit();
    });
</script>