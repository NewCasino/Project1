...
<ui:InputField ID="fldFixed" runat="server" Style="display:none">
    <ControlPart>
        ...
        <% var list = GmCore.GamMatrixClient.GetSupportedCurrencies();
            ...
            {
                var fees =  (this.ViewData["paymentMethod"] as PaymentMethod).DepositProcessFee.Currency2FixedFee; %>
                <tr>
...
<ui:InputField ID="fldBank" runat="server">
    <ControlPart>
        ...
        <% var list = GmCore.GamMatrixClient.GetSupportedCurrencies();
            foreach( var item in list)
            {
                var fees =  (this.ViewData["paymentMethod"] as PaymentMethod).DepositProcessFee.Currency2BankFee;
...
<script language="javascript" type="text/javascript">
    //$('#formDepositSaveProcessFee').initializeForm();
    ...
    $('#process-fee-type input[value="<%= (this.ViewData["paymentMethod"] as PaymentMethod).DepositProcessFee.ProcessFeeType %>"]').attr('checked', true);
    onTypeChanged();
    ...
    $('#btnSaveProcessFee').click(function (e) {
        ...
        $('#formDepositSaveProcessFee').ajaxForm(options);
        $('#formDepositSaveProcessFee').submit();
    });
</script>