...
<ul>
    ...
    <li>
        <a href="<%= this.Url.RouteUrl( "HistoryViewer", new {  
                        ...
                        @relativePath = "/.config/http_redirection.setting".DefaultEncrypt(),
                        @searchPattner = "",
...
<table id="table-redirection" class="table-list" cellpadding="0" cellspacing="0" rules="none" border="0" rules="rows">
    <thead>
        <tr>
...
<tr id="<#= arguments[0].guid#>">
    ...
    <td valign="middle" align="center" class="col-tools">
        <a href="javascript:void(0)" target="_self" onclick="self.tabHttpRedirection.onBtnRemoveClick('<#= arguments[0].guid#>')">Remove</a>
    </td>
</tr>
</script>

<% using (Html.BeginForm( "SaveHttpRedirectionRules"
       , null
       , new { @distinctName = this.Model.DistinctName.DefaultEncrypt() }
       , FormMethod.Post
       , new { @id = "formHttpRedirection"}
    ) ) { %>
...
function TabHttpRedirection(viewEditor) {
    self.tabHttpRedirection = this;

    this.getRulesAction = '<%= Url.RouteUrl( "RouteTable", new { @action = "GetHttpRedirectionRules", @distinctName = this.Model.DistinctName.DefaultEncrypt() }).SafeJavascriptStringEncode() %>';
    ...
    this.addNewRow = function (json) {
        ...
        $('#table-redirection tbody').append($('#rewrite-row-template').parseTemplate(json));
    };

    this.onLnkSaveClick = function () {
        $('#formHttpRedirection').html('');
        var rows = $('#table-redirection > tbody > tr');
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            if (url == '' || dest == '')
                continue;
            $('#formHttpRedirection').append('<input type="hidden" name="Key_' + total + '" value="' + url.htmlEncode() + '" />');
            $('#formHttpRedirection').append('<input type="hidden" name="Value_' + total + '" value="' + dest.htmlEncode() + '" />');
            total++;
        }
        $('#formHttpRedirection').append('<input type="hidden" name="total" value="' + total.toString(10) + '" />');
        ...
        var options = {
            ...
            success: function (json) {
                ...
                self.tabHttpRedirection.refresh();
            }
        };
        $('#formHttpRedirection').ajaxForm(options);
        $('#formHttpRedirection').submit();
    };
    ...
    this.refresh = function(){
        ...
        jQuery.getJSON(this.getRulesAction, null, function (json) {
            if( self.stopLoad ) self.stopLoad();
            if (!json.success) { alert(json.error); return; }
            $('#table-redirection > tbody').html('');
            for( var i = 0; i < json.data.length; i++){
                self.tabHttpRedirection.addNewRow(json.data[i]);
            }
        });
    };
    ...
    this.init = function () {
        ...
        $('#table-redirection > thead th.col-tools > a').bind('click', this, function (e) { e.data.onBtnAddNewRowClicked(); });

        $('#url-redirection-links a.save').bind( 'click', this, function(e){ e.data.onLnkSaveClick(); } );
        $('#url-redirection-links a.refresh').bind('click', this, function (e) { e.data.refresh(); });

        $('#url-redirection-links a.history').click(function (e) {
