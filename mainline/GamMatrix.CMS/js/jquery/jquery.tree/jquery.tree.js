    ...
    $.tree = {
        ...
        defaults: {
            data: {
                ...
                opts: { method: "GET", url: false, cache: false, distinctName: "" } // Options passed to datastore
                // allow use localStorage cache the data 
            },
            ...
            search: function (str, func) {
                ...
                if (this.settings.data.async) {
                    if (!this.srch_opn) {
                        ...
                        var jsonKey = "__Tree_" + this.settings.data.opts.distinctName;
                        var lsData = JSON.parse(localStorage.getItem(jsonKey));
                        if (lsData != null && this.settings.data.opts.cache) {
                            _this.srch_opn = $.unique(lsData.split(","));
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    _this.search.apply(_this, [str, func]);
                                    setLocalStorage(jsonKey, JSON.stringify(data));
                                }
                            });
                        }
                    }
        ...
        "html": function () {
            return {
                ...
                load: function (data, tree, opts, callback) {
                    if (opts.url) {
                        var jsonKey = "__Tree_" + opts.distinctName;
                        var lsData = JSON.parse(localStorage.getItem(jsonKey));
                        if (lsData != null && opts.cache) {
                            callback.call(null, lsData);
                        } else { 
                            $.ajax({
                                'type': opts.method,
                                'url': opts.url,
                                ...
                                'success': function (d, textStatus) {
                                    ...
                                    setLocalStorage(jsonKey, JSON.stringify(d));
                                },
            ...
            return {
                ...
                parse: function (data, tree, opts, callback) {
                    ...
                    if (tree.settings.languages.length) {
                        ...
                        str += ">";
                        if (data.data.icon) {
                            str += '<div class="' + data.data.cssClass + '"><div '
                            + (data.data.icon.indexOf("/") == -1 ? " class='"
                            + data.data.icon + "' " : " style='float:left; background-image:url(\"" + data.data.icon + "\"); background-repeat:no-repeat; background-position:left middle; padding-left:16px;' ") + ">" + data.data.title + "</div></div>";
                        }
                        else str += "<div><div>&nbsp;</div></div>";
                        str += "</a>";
                    }
...
function setLocalStorage(key, val) {
    try {
        ...
        for (var i = localStorage.length - 1   ; i >= 0; i--) { 
            if (localStorage.key(i).indexOf('_Tree') > 0) {
                localStorage.removeItem(localStorage.key(i));
                break;
            }
            if (i == 0) localStorage.clear();
        }
        localStorage.setItem(key, val);
    }
}