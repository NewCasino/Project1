...
unsafe public class WinFileIO : IDisposable
{
    ...
    public void OpenForReading(string FileName)
    {
        ...
        Close();
        pHandle = CreateFile(FileName, GENERIC_READ, 0x00000007, 0, OPEN_EXISTING, 0, 0);
    ...
    public static string ReadWithoutLock(string filePath)
    {
        ...
        WinFileIO WFIO = new WinFileIO(ByteBuf);
        WFIO.OpenForReading(filePath);
        string content = string.Empty;
        //i do this because the first 3 bytes are garbage 
        int bytesRead;
        int nodo = 0;
        do
        {
            if (nodo == 0)
            {
                bytesRead = WFIO.Read(buffer, 0, buffer.Length);
                if ((bytesRead > 3) && (buffer[0] == 239))
                    content += Encoding.UTF8.GetString(buffer, 3, bytesRead - 3);
                else if (bytesRead > 3)
